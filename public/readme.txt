...
Universal Live Tracking and Recording Appは、インターネット上に公開されている動画配信サイト等の動画をダウンロードしたり、特定のユーザが配信を始めた際に通知させたりできるソフトウェアです。
...
通知方法を設定するには、メニューバーの[オプション]→[設定]を選択し、[通知]タブを使用します。

３．２　録画機能
通知方法として[録画]が有効になっている場合に、配信をリアルタイムに録画できます。
録画したデータはMP4などのファイル形式で保存できるほか、音声のみを保存することも可能です。
...
・「録画」として公開されている動画のダウンロード
・ユーザ名を指定して録画
なお、ツイキャス連携機能を使用するためには、ツイキャスのアカウントが必要です。
...
[専用設定を使用]を有効にすると、専用の通知方法を設定できます。
...
・ユーザ情報を更新：ツイキャスでは、ユーザが自由にユーザ名や名前を変更できます。この項目を選択すると、最新のユーザ情報を取得し、ユーザ名や名前が変更されている場合にはそれを画面表示に反映します。ただし、配信開始を検知した際に自動でユーザ情報を更新するようになっているため、通常は手動でこの機能を呼び出す必要はありません。
・指定したユーザのライブを録画：指定したユーザを、録画対象として追加します。この情報は、登録後一定時間経過後に自動で削除されます。ここで指定したユーザがライブを開始した場合、バルーン通知等は行わずに録画を開始します。
...
２．　ツイキャスには、「グループ限定配信」や「プライベート配信」など、特定のユーザにしか閲覧できない特別な配信があります。本ソフトの通知機能は、これらの配信には対応していません。ただし、合い言葉で保護された録画をダウンロードすることは可能です。この場合、「合い言葉」の入力が必要です。
