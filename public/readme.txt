...
・過去ライブのダウンロード：「録画」としてツイキャスのサイトに公開されている動画をダウンロードできます。ダウンロードを開始するには、この項目を選択して現れるエディットボックスに、再生ページのURLを入力します。また、ULTRAと連携しているアカウントが適切な閲覧権限を持っていれば、プレミア配信の録画などもダウンロードできます。なお、この機能を使用するにはログインが必要です。「ログイン状態で録画」が無効になっている場合、この項目の実行時にその場でログインを求められます。ただし、いくつかの制約があります。詳細については、次節の「利用に当たっての注意事項」を参照してください。
・過去ライブの一括ダウンロード：指定したユーザの過去ライブの内、「録画」として公開され、合い言葉で保護されていないものをすべてダウンロードします。この項目を選択して現れるエディットボックスにユーザ名を入力すると、自動的にダウンロードが行われます。なお、すでに本ソフトで録画されているライブはダウンロードされません。この機能を使用するには、あらかじめ「ログイン状態で録画」を有効にしてログインしておく必要があります。ログインしていない状態でこの項目を選択すると、エラーメッセージが表示され、処理は行われません。
...
・録画画質の設定：ツイキャスから録画用の動画URLを取得する際に、どの画質を使用するかを設定します。詳細は、「４．１．6　画質設定について」をご参照ください。
...
４．　[過去ライブのダウンロード]機能を使用するには、ログインが必要です。「ログイン状態で録画」を有効にしていない状態でこの機能を実行すると、「過去ライブをダウンロードするには、ログインする必要があります。今すぐログインしますか？」というメッセージが表示されます。プレミア配信の録画など、特定のアカウントでしか閲覧できないライブについては、その詳細情報を取得することができないため、代わりに情報の取得に失敗した旨を通知するメッセージが表示されます。いずれの場合も、[はい]を選択すると、ULTRAと連携しているアカウントのパスワードの入力が求められます。正しいパスワードを入力し、ログインに成功すると、改めてライブのダウンロードを試みます。1度ログインに成功すると、そのユーザ名とログイン情報（セッション）を保存するため、使用するアカウントを変更しない限り、パスワードを再度入力する必要は通常ありません。また、情報の取得に失敗したライブをこの方法で録画した場合、配信開始日時を取得できないため、録画を開始した時点での日時がファイル名に使用されます。また、ファイル名の設定内容に関係なく、ファイル名の末尾にライブのID（数値）を追加します。なお、[過去ライブの一括ダウンロード]機能では、その場でのログインには対応していません。あらかじめ「ログイン状態で録画」を有効にしてログインしておく必要があります。
...
４．１．6　画質設定について
ツイキャスでは、配信者の環境や設定によって、高画質版・中画質版・低画質版の複数の画質で配信が行われることがあります。
ULTRAで録画を行う際、これらの内どの画質を使用するかを選択できます。
...
通信環境に制約がある場合や、ストレージ容量を節約したい場合には、低画質版や中画質版をご使用ください。
