		...
		if account not in data.keys():
			...
			if type(session) == int:
				...
				messages = {
					...
					errorCodes.LOGIN_TWITTER_WRONG_ACCOUNT: _("Twitterユーザ名またはパスワードが不正です。設定を確認してください。なお、ULTRA Version 1.5.2の更新に伴い、ツイキャスの「ログインパスワード」を使用してログインするようになりました。ツイキャスのサイトでログインパスワードを設定し、そのパスワードをULTRAに設定する必要があります。"),
